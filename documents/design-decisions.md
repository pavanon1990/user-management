...
- กลุ่มที่มี library จะอยู่ที่ `internal/adapter`, `internal/infra` และภายใต้ `pkg`  เพื่อไม่ให้ Library อยู่ใน layter service logic ป้องกัน 
- นำมาประกอบกันที่ `cmd/main.go` โดยใช้หลักการ dependency injection